...
- **Парсеры — в коде, не в БД.** Сервисы добавляет разработчик (редко, пару раз в год). Таблица `services` хранит только метаданные (id/name/форматы); логика парсинга — функции в `app.py` + авто-синк `SERVICES → services` при старте. Config-driven движок (логика парсинга в строках БД + редактор в UI) сознательно отклонён как переинженеринг (inner-platform effect): теряется git-история/review/тесты, ошибки видны только в рантайме, а первый нестандартный формат всё равно требует кода.
